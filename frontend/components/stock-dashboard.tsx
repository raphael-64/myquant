...
import { getStockData, getStockSentiment } from "@/lib/actions"
...
export default function StockDashboard({ stockSymbol }: StockDashboardProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch price data
        const priceResponse = await getStockData(stockSymbol)
        setPriceData(priceResponse.data)

        // Fetch sentiment data
        const sentimentResponse = await getStockSentiment(stockSymbol)
  ...
  const currentSentiment = sentimentData.length > 0 ? sentimentData[sentimentData.length - 1].sentiment : 0
  ...
  return (
    <div className="grid gap-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>News and social media analysis</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <div className="flex items-center mt-1">
                  <Badge
                    variant={currentSentiment > 0.3 ? "success" : currentSentiment < -0.3 ? "destructive" : "outline"}
                  >
                    {currentSentiment > 0.3 ? "Positive" : currentSentiment < -0.3 ? "Negative" : "Neutral"}
                  </Badge>
                  <span className="ml-2 text-sm">{(currentSentiment * 100).toFixed(1)}%</span>
                </div>
              </div>
              <div>
                <div className="text-sm text-muted-foreground">Articles Analyzed</div>
                <div className="text-xl font-medium mt-1">
                  {sentimentData.length > 0 ? sentimentData[sentimentData.length - 1].articles : 0}
                </div>
              </div>
            </div>
      ...
      <AgentDecision stockSymbol={stockSymbol} priceData={priceData} sentimentData={sentimentData} />
    </div>
  )
}
