...
export default function AgentWeightsChart({
  stockSymbol,
}: AgentWeightsChartProps) {
  ...
  useEffect(() => {
    ...
    if (mockData.length > 0) {
      ...
      setCurrentWeights([
        {
          name: "Mean Reversion",
          value: lastData.meanReversion,
          color: "hsl(var(--chart-1))",
        },
        {
          name: "Momentum",
          value: lastData.momentum,
          color: "hsl(var(--chart-2))",
        },
        {
          name: "Sentiment Momentum",
          value: lastData.sentimentMomentum,
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      setCurrentWeights((prevWeights) => {
        ...
        const totalChange =
          meanReversionChange + momentumChange + sentimentChange;
        meanReversionChange -= totalChange / 3;
        momentumChange -= totalChange / 3;
        sentimentChange -= totalChange / 3;

        // Apply changes with constraints
        newWeights[0].value = Math.max(
          0.1,
          Math.min(0.6, newWeights[0].value + meanReversionChange)
        );
        newWeights[1].value = Math.max(
          0.1,
          Math.min(0.6, newWeights[1].value + momentumChange)
        );
        newWeights[2].value = Math.max(
          0.1,
          Math.min(0.6, newWeights[2].value + sentimentChange)
        );

        // Normalize to ensure sum is exactly 1.0
        const sum =
          newWeights[0].value + newWeights[1].value + newWeights[2].value;
  ...
  const modernColors = ["#6FCF97", "#56CCF2", "#F2C94C"];
    ...
    <div className="grid gap-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Self-learning system weight adjustments for {stockSymbol}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Tabs defaultValue="pie">
            ...
            <TabsContent value="pie">
              <div className="h-[100px] flex flex-col items-center justify-center w-full">
                {/* The big horizontal bar */}
                <div className="flex w-full h-20 overflow-hidden rounded-lg border border-muted">
          ...
          <div className="mt-6">
            ...
            <p className="text-sm text-muted-foreground">
              The system automatically adjusts agent weights based on prediction
              accuracy. When an agent consistently makes accurate predictions,
              its weight increases. Conversely, when an agent's predictions are
              less accurate, its weight decreases. This is the learning process
              of this agentic system.
            </p>
          </div>
        </CardContent>
      </Card>
